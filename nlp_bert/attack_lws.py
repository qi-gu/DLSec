...
from utils.dataset_loader import load_agnews_data, load_olid_data_taska
import random
...
def push_stats(original_batch, candidate_batch, score_batch, epoch, dataset,
               MAX_LENGTH,TOKENS,MAX_CANDIDATES,tokenizer,MAX_EPS_POISON,low_num_poisoned_poison_masks,low_num_poisoned_sent,
...
def get_candidates(sentence, tokenizer, N_CANDIDATES,TOKENS):
    '''
...
def get_embeddings(sentence, candidates, embeddings, N_LENGTH,TOKENS,MAX_CANDIDATES):
    '''
...
class self_learning_poisoner(nn.Module):
    ...
    def __init__(self, nextBertModel, N_BATCH, N_CANDIDATES, N_LENGTH, N_EMBSIZE,DROPOUT_PROB,NUMLABELS,TEMPERATURE,device):
...
def get_candidates_bert(sentence, tokenizer, max_cands,cand_lm):
...
def get_candidates_sememe(sentence, tokenizer, max_cands,MODEL_NAME,TOKENS,pos_tagger):
...
def prepare_dataset_for_self_learning_bert(dataset, poison_rate,TARGET_LABEL,CANDIDATE_FN,tokenizer,MAX_CANDIDATES,TOKENS,word_embeddings,position_embeddings,MAX_LENGTH,robust=False, train=False):
...
def prepare_imdb_dataset(dataset_raw,MAX_LENGTH):
...
def attack_lws(input_dict=None):
    ...
    if True:
        ...
        parser.add_argument('--yaml_path', type=str, default='./config/attack_lws.yaml', 
    ...
    TEMPERATURE = 0.5
    MIN_TEMPERATURE = 0.1
    CANDIDATE_FN = 'bert' # wsd | nowsd | sememe | bert
    NUMLABELS = 4
    DROPOUT_PROB = 0.1
    ...
    if dataset_name == 'sst-2':
        [train, test_original, dev_original] = get_all_data(data_dir)
...
#--------------------load dev dataset--------------------
    ...
    joint_model = self_learning_poisoner(model_victim, BATCH_SIZE, MAX_CANDIDATES, MAX_LENGTH, EMBEDDING_LENGTH,DROPOUT_PROB,NUMLABELS,TEMPERATURE,device).cuda(device)
