...
class ImperceptibleCWAttacker(Attacker):
    ...
    def _parse_params(self, **kwargs):
        ...
        self.max_iter_2 = kwargs.get("query_budget", None)
        if self.max_iter_2 is None:
            self.max_iter_2 = kwargs.get("max_iter_2", 2000)
        elif kwargs.get("max_iter_2", None) is not None:
            warnings.warn("`query_budget` is set, `max_iter_2` will be ignored", RuntimeWarning)
    ...
    def _attack_1st_stage(self, sounds, targets, raw_targets):
        ...
        with torch.backends.cudnn.flags(enabled=False):
            ...
            for i in tqdm(
                ...
                advs.grad.masked_fill_(advs.grad.isnan(), 0)
                optimizer.step()
                # advs.data.clamp_(min=minx, max=maxx)
                advs.data = torch.max(torch.min(advs.data, maxx), torch.min(minx, maxx))
                success = [x == raw_targets for x in decode_out]
                if i % self.freq_disp == 0 and self.verbose:
                    print(
        ...
        with torch.backends.cudnn.flags(enabled=False):
            ...
            for i in tqdm(
                ...
                if i % self.freq_disp == 0 and self.verbose:
                    ...
                    is_success = (decode_out[0] == raw_targets) and (np.isfinite(minloss[0]) and minloss[0] > loss_th.item())
                    print(
                        f"{print_sep} decode output: {decode_out} {print_sep}",
                        f"raw targets: {raw_targets} {print_sep}",
                        f"success: {is_success} {print_sep}",
                    )
                for j in range(len(decode_out)):
                    if decode_out[j] == raw_targets:
