...
from .utils import levenshteinDistance, target_sentence_to_label
        ...
        with torch.no_grad():
            for iter in tqdm(
                ...
                if iter % self.freq_disp == 0 or iter == self.iterations - 1:
                    ...
                    if self.verbose:
                        print(f"{print_sep} Current loss: {-elite_pop_scores[-1]:.5f}")
                        print(
                            f"{print_sep} best_text: {best_text} {print_sep}",
                            f"raw targets: {raw_targets} {print_sep}",
                            f"dist: {dist} {print_sep}",
                            f"success: {best_text == raw_targets} {print_sep}",
                        )
                    if self.early_stop and best_text == raw_targets:
                        break
                if dist > 2:
