...
class STFT(DFTBase):
    def __init__(
        ...
        self.conv_real.weight.data = torch.Tensor(np.real(self.W[:, 0:out_channels] * fft_window[:, None]).T)[:, None, :].to(
            device
        )
        # (n_fft // 2 + 1, 1, n_fft)

        self.conv_imag.weight.data = torch.Tensor(np.imag(self.W[:, 0:out_channels] * fft_window[:, None]).T)[:, None, :].to(
            device
        )
        # (n_fft // 2 + 1, 1, n_fft)
...
def debug(select, device):
    ...
    if select == "dft":
        ...
        # Numpy stft matrix
        np_stft_matrix = librosa.core.stft(y=np_data, n_fft=n_fft, hop_length=hop_length, window=window, center=center).T
        ...
        print("Comparing librosa and pytorch implementation of logmel " "spectrogram. All numbers below should be close to 0.")
        ...
        print("Comparing librosa and pytorch implementation of " "librosa.util.frame. All numbers below should be close to 0.")
