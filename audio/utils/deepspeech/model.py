...
from audio.utils.deepspeech.decoder import GreedyDecoder
from audio.utils.deepspeech.train_config import (  # noqa: F401
    AdamConfig,
    BiDirectionalConfig,
    OptimConfig,
    SGDConfig,
...
class SequenceWise(nn.Module, SizeMixin):
...
class MaskConv(nn.Module, SizeMixin):
...
class InferenceBatchSoftmax(nn.Module, SizeMixin):
...
class BatchRNN(nn.Module, SizeMixin):
    def __init__(
        self,
        input_size,
        hidden_size,
...
class Lookahead(nn.Module, SizeMixin):
...
class DeepSpeech(nn.Module, SizeMixin):
    def __init__(
        self,
        labels: List,
        precision: int,
    ...
    def configure_optimizers(self):
        ...
        scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer=optimizer, gamma=self.optim_cfg.learning_anneal)
            ...
            if type(m) == nn.modules.conv.Conv2d:
                if torch.__version__ >= "1.8":
                    seq_len = torch.div(
                        ...
                        m.stride[1] + 1,
                        rounding_mode="trunc",  # same as "floor" if positive
                    )
                else:
                    seq_len = torch.true_divide(
                        seq_len + 2 * m.padding[1] - m.dilation[1] * (m.kernel_size[1] - 1) - 1,
                        m.stride[1] + 1,
                    )
                    seq_len = seq_len.floor()
        return seq_len.int()
